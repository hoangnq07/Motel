...
public class MotelDAO {
    ...
    public static void  updateMotel(Motel motel) throws SQLException {
        String sql = "UPDATE dbo.motels SET descriptions = ?, detail_address = ?, district = ?, image = ?, province = ?, status = ?, ward = ?, account_id = ? ,name =?,district_id =?, province_id=? WHERE motel_id = ?";
        String sql1 = "UPDATE dbo.motels SET descriptions = ?, detail_address = ?, image = ?, status = ?, account_id = ? ,name =? WHERE motel_id = ?";
        try (Connection conn = DBcontext.getConnection();) {
            PreparedStatement stmt = conn.prepareStatement(sql);
            if(!(motel.getProvince()==null||motel.getProvince().isEmpty())){
                ...
                stmt.setString(10, motel.getDistrictId());
                stmt.setString(11, motel.getProvinceId());
                ...
                stmt.setString(1, motel.getDescriptions());
                stmt.setString(2, motel.getDetailAddress());
                stmt.setString(3, motel.getImage());
                stmt.setBoolean(4, motel.isStatus());
                stmt.setInt(5, motel.getAccountId());
                stmt.setString(6, motel.getName());
                stmt.setInt(7, motel.getMotelId());
                stmt.executeUpdate();
            }
        }
    }
